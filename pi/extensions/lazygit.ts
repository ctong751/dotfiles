 ...
 * Lazygit Extension
 *
 * /lg and /diff temporarily suspend pi's TUI and launch lazygit in the
 * current repository.
 */
...
export default function (pi: ExtensionAPI) {
	const openLazygit = async (_args: string, ctx: ExtensionCommandContext) => {
		if (!ctx.hasUI) {
			ctx.ui.notify("lazygit command requires interactive mode", "error");
			return;
		}
